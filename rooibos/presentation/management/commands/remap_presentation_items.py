...
class Command(BaseCommand):
    option_list = BaseCommand.option_list + (
        make_option('--from', '-f', dest='from_collection', action='store', type='int', help='Source collection'),
        make_option('--to', '-t', dest='to_collections', action='append', type='int', help='Target collection'),
        make_option('--commit', '-c', dest='commit', action='store_true', default=False, help='Commit changes'),
    )
    help = "Maps presentation items from records in one collection to records in another collection. " + \
           "Records are compared by their identifier."


    def handle(self, from_collection, to_collections, commit, *args, **options):

        if not from_collection or not to_collections:
            print "Error: Must specify --from and --to arguments"
            return

        print "Mapping presentation items from collection %s to collection(s) %s" % (from_collection, to_collections)
        ...
        items = PresentationItem.objects.filter(record__collectionitem__collection=from_collection)
                ...
                for identifier in identifiers:
                    ...
                    if new_records:
                        if len(new_records) == 1:
                            ...
                            errors.append("Multiple matching records with identifier '%s' found in collection %s: %s" % (identifier, to_collections, sorted(new_records)))
                    else:
                        errors.append("No record with identifier '%s' found in collection %s" % (identifier, to_collections))
            else:
                errors.append("No identifier found for record %s" % item.record_id)
            pb.update(count)
        ...
        if commit:
            ...
            print "Would have remapped %s items - rerun with --commit" % remapped
        if errors:
