package com.adobe.wheelerstreet.fig.panzoom.utils
{
	...
	public class ContentRectangle extends Rectangle
	{
		...
		public function getFitToRectangleXandY(rect:Rectangle):Point {
			var _scaleRatioX:Number  =  _unscaledWidth  / rect.width;
			var _scaleRatioY:Number  =  _unscaledHeight / rect.height;
			var p:Point = new Point();
			
			// is content wider or thinner than view?
			if (_scaleRatioX < _scaleRatioY) {
				// if  thinner.
				var yRatio:Number = rect.height/_unscaledHeight							
				p.x = yRatio;
				p.y = yRatio;
				
			} else {			
				// if wider.
				var xRatio:Number = rect.width/_unscaledWidth;
				p.x = xRatio;
				p.y = xRatio;
			}
			return p;
