package flex.utils.spark.resize {
	...
	public class ResizeManager extends EventDispatcher {
		...
		private var _hostComponentContainer:UIComponent;

		private var _constrainToHostComponentBounds:Boolean;
		...
		public function ResizeManager(resizeComponent:UIComponent = null, resizeHandle:UIComponent = null, resizeDirection:String = "both") {
			...
			//this._hostComponentContainer = this.resizeComponent["catalogWindowContainer"] as UIComponent;
		...
		public function set hostComponentContainer(container:UIComponent):void {
			this._hostComponentContainer = container;
		}
		
		public function get hostComponentContainer():UIComponent {
			return this._hostComponentContainer;
		}
		 ...
		 */
		public function get constrainToHostContainerBounds():Boolean {
			return _constrainToHostComponentBounds;
		}

		/**
		 * Set to true to constrain the resizing to keep the resize component inside the parent bounds.
		 */
		public function set constrainToHostContainerBounds(value:Boolean):void {
			_constrainToHostComponentBounds = value;
		}
		...
		private function resizeMouseMoveHandler(event:MouseEvent):void {
			...
			// constrain the size to keep the resize component inside the host container (modified by K. Hegg) bounds
			if (constrainToHostContainerBounds && _hostComponentContainer) {
				var globalCoordinates:Point = _hostComponentContainer.localToGlobal(new Point(_hostComponentContainer.x, _hostComponentContainer.y));
				var parentWidth:Number = _hostComponentContainer.width;
				var parentHeight:Number = _hostComponentContainer.height;
				if ((resizeComponent.x + newWidth) > globalCoordinates.x + parentWidth) {
					newWidth = parentWidth - resizeComponent.x;
				}
				if ((resizeComponent.y + newHeight) > globalCoordinates.y + parentHeight) {
		...
		private function setResizeCursor():void {
			if ((resizeCursorID == 0) && (resizeCursorIcon != null)) {
				resizeCursorID = _hostComponentContainer.cursorManager.setCursor(resizeCursorIcon);
			}
		}

		private function removeResizeCursor():void {
			if (resizeCursorID != 0) {
				_hostComponentContainer.cursorManager.removeCursor(resizeCursorID);
				resizeCursorID = 0;
			}
		}
