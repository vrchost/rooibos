package flex.utils.spark.resize {
	...
	public class MoveManager {
		...
		private var _constrainToHostContainerBounds:Boolean;
		...
		public function MoveManager(moveComponent:UIComponent = null, dragComponent:UIComponent = null) {
			...
			_constrainToHostContainerBounds = false;
			moveCursorID = 0;
		...
		public function get constrainToHostContainerBounds():Boolean {
			return _constrainToHostContainerBounds;
		}
		...
		public function set constrainToHostContainerBounds(value:Boolean):void {
			_constrainToHostContainerBounds = value;
		}
		...
		private function dragComponentMouseDown(event:MouseEvent):void {
			...
			// Constain the movement by the host component's bounds?
			if (constrainToHostContainerBounds && hostComponentContainer) {
				var bounds:Rectangle = new Rectangle(0, 0, hostComponentContainer.width, hostComponentContainer.height);
		...
		private function setMoveCursor():void {
			...
			if ((moveCursorID == 0) && (moveIcon != null)) {
				moveCursorID = hostComponentContainer.cursorManager.setCursor(moveIcon, 2, -12, -10);
			}
		}

		private function removeMoveCursor():void {
			if (moveCursorID != 0) {
				hostComponentContainer.cursorManager.removeCursor(moveCursorID);
				moveCursorID = 0;
			}
		}
